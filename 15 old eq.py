...
##        print "Theta", self.tanX
        ...
        self.velocity=u*(4.0/3)#calculates the velocity of this stationary self piece due to collision with a moving particle
        #if self.velocity>50:  
            #self.velocity=0
...
def computer(): #of the striker position
    ...
    for y in range(250,700,5):
        ...
        if len(piecelist)>1 and shortest1>shortest2: #checking which of the two close pieces is the closest
...
def paused():
    ...
    TextSurf, TextRect = text_objects("Paused", largeText,lightred)
...
options = [Option("RULES", (125, 200),smallfont),Option("CARROM BOARD", (400, 30),mediumfont), Option("SINGLE PLAYER", (125, 255),smallfont),
   Option("MULTI PLAYER", (125, 305),smallfont),Option("FRICTION", (125, 355),smallestfont)]
                ...
                for otherpiece in piecelist:
                    if piece!=otherpiece:
                        if otherpiece.difference(piece.x,piece.y)!=None:
                            distance=difference(piece.x,piece.y)
