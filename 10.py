...
class piece(): #this is the class for all the carrom pieces   
    ...
    def residual(self,u): #the forward velocity of the piece which collided with this self piece
        sinX2,cosX2,tanX2 = self.cosX,self.sinX,1/(self.tanX)  #wrong# sinX2=90-cosX1, cosX2=90-sinX1, tanX2=90-tanX1
...
iscomputer=False #initialisng iscomputer for SIngle Player
...
def main(): #for displaying messages on screen #pygame documentation   
    ...
    while intro:
        for event in pygame.event.get():
            if event.type==pygame.QUIT: #checking if the cross key on the window is not pressed
                pygame.quit()
                quit()
        window.fill(white)
        ...
        pygame.event.pump() #loads the events of pygames
        for option in options:  #Loops over the list options
                ...
                for event in pygame.event.get():
                    if event.type == pygame.KEYDOWN: # if the enter key is pressed
                        if event.key==pygame.K_RETURN:
                            ...
                            if x<350 and x>125 and y>255 and y<300: #checking if it was pressed on single player
                                intro=False #this while loop is terminated and it moves on to other loops
                                iscomputer=True 

                    if event.type == pygame.KEYDOWN: # if the enter key is pressed
                        if event.key==pygame.K_RETURN:
                            x,y = pygame.mouse.get_pos() #checks where the enter key was pressed
                            if x<350 and x>125 and y>300 and y<350: #if it were pressed in this region that correponds to multi player, this while loop is terminated
                                intro=False
                                
                    if event.type == pygame.KEYDOWN: # if the enter key is pressed       
                        if event.key==pygame.K_RETURN:
                            x,y = pygame.mouse.get_pos() #checks where the mouse was clicked
                            if x<500 and x>125  and y>350 and y<400:
                                options.append(Option("friction=1", (125,400),mediumfont)) #make new labels for
